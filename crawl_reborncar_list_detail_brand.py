    ...
    try:
            ...
            if car_infos.count() >= 2:
                ...
                if plan_ul.count() > 0:
                    ...
                    for li in lis:
                        ...
                        # "리스 137만원 (48개월)" → "리스 : 137만원 (48개월)"
                        if txt:
                            parts.append(re.sub(r"\s+", " : ", txt, count=1) if re.search(r"\d", txt) else txt)
        ...
        for i in range(info_list.count()):
            try:
                ...
                elif "신차가격대비" in title: detail_data["comp_new_car"] = value
...
def run_full_crawler():
    ...
    TEST_PAGE_LIMIT = 5   # ← 테스트 시 이 줄 주석 해제하고 아래 줄 주석 처리
    ...
    list_headers = [
        "model_sn", "productId", "boname", "gradename", "year", "carnavi", "car_seat",
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            while True:
                ...
                logger.info(f"현재 {current_page}페이지 수집 중... (매물 {len(items)}개)")
                ...
                for item in items:
                    try:
                        ...
                        # list.csv 행 (목록 데이터만)
                        list_row = {
                            "model_sn": car_counter, "productId": v_product_id,
                        ...
                        if v_product_id and v_status not in ["준비중", "판매완료"]:
                            # logger.info(f"상세 수집 중: {current_page}페이지, productId {v_product_id}")
                ...
                # [테스트] TEST_PAGE_LIMIT 설정 시 목록 N페이지에서 종료 → 상세도 그만큼만 수집됨
