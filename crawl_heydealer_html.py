...
def _extract_detail_smart(page) -> dict:
    """상세 페이지 정밀 추출 (span 데이터 유무 기반 분류 & 라벨 매칭)"""
    ...
    try:
        ...
        time.sleep(2.5) # year 데이터 누락 방지를 위한 충분한 대기

        # 1. 모델/등급 추출 (span 유무 로직)
        ...
        if header:
            ...
            if len(span_texts) == 1:
                ...
                if span_texts[0] and span_texts[1]: # 둘 다 데이터 있을 때
                    res["model_second_name"] = span_texts[0]
                    res["grade_name"] = span_texts[1]
                else: # 하나만 있을 때 (일반적으로 첫번째)
                    res["grade_name"] = span_texts[0]

        # 2. 7종 스펙 라벨 매칭 (year ~ insurance 밀림 방지)
...
def main():
    ...
    with sync_playwright() as p:
        ...
        # list.csv 저장
        list_fields = ["car_sn", "used_car_site", "model_name", "model_second_name", "grade_name", "year", "km", "before_sale", "sale_price", "new_car_price", "detail_url"]
        with open(RESULT_DIR / "heydealer_cars_list.csv", "w", newline="", encoding="utf-8-sig") as f:
            ...
            for i, c in enumerate(raw_cars, 1):
                writer.writerow({"car_sn": i, "used_car_site": "헤이딜러", "detail_url": c["url"], **{k: c[k] for k in list_fields[2:10]}})

        # 2. 상세 수집 (raw_cars 미정의 에러 해결)
        detail_fields = ["sn", "car_sn", "used_car_site", "model_name", "model_second_name", "grade_name", "images_all", "year", "km", "refund", "guarantee", "accident", "inner_car_wash", "insurance", "before_sale", "sale_price", "new_car_price"]
            ...
            for i, c in enumerate(raw_cars, 1):
                ...
                try:
                    ...
                    writer.writerow({
                        ...
                        "sale_price": c["sale_price"], "new_car_price": c["new_car_price"],
                        **{k: det[k] for k in ["images_all", "year", "km", "refund", "guarantee", "accident", "inner_car_wash", "insurance"]}
                    })
